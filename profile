...
function gen_backend() {
  ...
  openstack server create --image cirros-0.3.5-x86_64-disk --flavor c1 --nic net-id=$PRIVATE_NETWORK member1 --security-group member --key-name default
  openstack server create --image cirros-0.3.5-x86_64-disk --flavor c1 --nic net-id=$PRIVATE_NETWORK member2 --security-group member --key-name default --wait
  sleep 15
